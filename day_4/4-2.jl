...
function day_4_part_2(boards :: Vector{BingoBoard}, numbersCalled :: Vector{Int})
    mostRecentScore = 0
    for number in numbersCalled
        indicesToDelete = Vector{Int}()
        for (i, board) in enumerate(boards)
            markBoardIfNumberPresent!(board, number)
            if (hasBingoed(board))
                mostRecentScore =  sumUnmarkedValues(board) * number
                push!(indicesToDelete, i)
            end
        end
        deleteat!(boards, indicesToDelete)
        if (length(boards) == 0)
...
println(day_4_part_2(boards, numbersCalled))